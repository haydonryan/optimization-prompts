...
After any candidate is accepted (and its story created), do NOT stop at that
candidate:
...
- **Cross-apply the pattern to other similar sites.** When a prompt finds an
  ...
  A/B test each individually (full LTO × arch matrix). The first hit is rarely the
  only instance.
- Each cross-apply candidate that passes the matrix becomes its own story, exactly
  like a first-pass candidate.
